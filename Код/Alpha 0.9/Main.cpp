...
int main()
{
	...
	for (int j = 0; j < 1;)
	{
		...
		if (data == 1)
		{
			cout << "1 - Домашние или рабочие компьютеры, 2 - Смартфоны, 3 -"/*VR гарнитуры*/"(ERROR): ";
			cin >> data;
			if (data == 1)
			{
				for (int i = 0; i < 1;)
		...
		else if (data == 3)
		{
			...
			cout << "Лицензия RTM (Отдельно) - Debug (Alpha 0.9 or PreAlpha 0.3.4)" << endl << endl << "MaxHorror5993 PCsupport. Copyright (R)." << endl << endl << endl << endl;
		}
		else if (data == 4)
			j++;
		else if (data >= 5)
